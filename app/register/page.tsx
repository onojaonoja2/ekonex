...
export default async function RegisterPage({
    ...
    searchParams: Promise<{ role?: string, error?: string, courseId?: string, next?: string }>
}) {
    const { role = 'student', error, courseId, next } = await searchParams
    const isInstructor = role === 'instructor'

    const redirectPath = next || (courseId ? `/courses/${courseId}` : null)
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
            ...
            <div className="w-full max-w-md">
                ...
                <form className="glass rounded-3xl p-8 border border-white/5 shadow-2xl">
                    ...
                    {redirectPath && <input type="hidden" name="redirectUrl" value={redirectPath} />}
                    ...
                    <div className="mt-6 text-center text-sm text-slate-500">
                        ...
                        <Link href={redirectPath ? `/login?next=${encodeURIComponent(redirectPath)}` : '/login'} className="text-white hover:underline">Log in</Link>
                    </div>
                </form>
            </div>
        </div>
